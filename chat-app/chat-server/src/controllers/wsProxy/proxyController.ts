...
import { buildWsUserMessage, UserOperation, ChatOperation, buildWsChatMessage } from "../../wsProtocol";
...
export const broadcastAddUser = (userName: string, userKV: KV) => {
    const addUserMsg = buildWsUserMessage(
...
export const broadcastRemoveUser = (userName: string) => {
    const removeUserMsg = buildWsUserMessage(
...
export const broadcastUserOnline = (userName: string) => {
    const onlineMsg = buildWsUserMessage(
        UserOperation.Online,
        { userName: userName },
    );
...
export const broadcastUserOffline = (userName: string) => {
    const offlineMsg = buildWsUserMessage(
        UserOperation.Offline,
        { userName: userName },
    );
...
export const broadcastNewMessage = (messageKV: KV) => {
    const newMsg = buildWsChatMessage(
        ChatOperation.NewMessage,
        messageKV,
    );
