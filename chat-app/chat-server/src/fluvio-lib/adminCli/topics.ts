...
export const createTopicIfNotDefined = async (
  name: string,
  partitions: number,
  replication: number
) => {
...
export const createTopic = async (
  ...
  let cmd = `fluvio topic create -r ${replication} -p ${partitions} ${name}`;
...
export const hasTopic = async (name: string) => {
  let cmd = `fluvio topic describe ${name}`;
  let cmdRes = (await execShellCommand(cmd)) as string;

  if (cmdRes.indexOf("Retrieved 0 out of 0 topics") >= 0) {
    return false;
  }
...
export const getTopic = async (name: string) => {
  let cmd = `fluvio topic describe -O json ${name}`;
  let cmdRes = (await execShellCommand(cmd)) as string;
...
export const deleteTopic = async (name: string) => {
  let cmd = `fluvio topic delete ${name}`;
  let cmdRes = (await execShellCommand(cmd)) as string;
